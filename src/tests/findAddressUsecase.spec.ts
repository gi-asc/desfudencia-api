...
describe('CalculateMedia', () => {
    ...
    it('should return an error message if the neighborhood comes empty', async () => {
        const cep = "35930040"
        const spy = jest.spyOn(repository, 'findAddress').mockImplementationOnce(async (cep) => {
            return {
                    cep: "35930-040",
        ...
        const response = await findAddressUsecase.execute(cep)
        const expetedMessage = Object.assign(response, {message: "could not find the neighborhood"})
